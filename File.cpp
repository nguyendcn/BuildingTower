...
void File(int hightScore)
{
	...
	if (FileIn.fail()) // Thất bại => lần đầu play;
	{
		...
		do {
			gotoxy(45, 22); cout << "Enter the name(length < 7): ";
			cin >> s.name;
			if (s.name.size() >= 7)
			{
				gotoxy(45, 22);  cout << "                                        ";
...
void FileScore(int hightScore)
{
	system("cls");
	HienConTro();
	int xEnterName = 40, yEnterName = 15;
	vector<Score> topScore;
	...
	if (FileIn.fail()) // Thất bại => lần đầu play;
	{
		...
		ofstream FileOut("hightScore.in", ios::out); // Tạo mới file hight socre;
		do {
			gotoxy(xEnterName, 15); cout << "Enter the name(length < 7): ";
			cin >> s.name;
			if (s.name.size() >= 7)
			{
				gotoxy(xEnterName, yEnterName);  cout << "                                        ";
			}
		} while (s.name.size() >= 7);
	...
	Score s;
	do {
		gotoxy(xEnterName, yEnterName); cout << "Enter the name(length < 7): ";
		cin >> s.name;
		if (s.name.size() >= 7)
		{
			gotoxy(xEnterName, yEnterName);  cout << "                                        ";
